...
class Solution {
    public int countPalindromicSubsequence(String s) {
        ...
        // Iterate over each character from 'a' to 'z' as potential first and last character
        for (char ch = 'a'; ch <= 'z'; ch++) {
            int left = s.indexOf(ch);  // First occurrence
            int right = s.lastIndexOf(ch); // Last occurrence

            if (right - left > 1) { // Valid range for middle characters
                HashSet<Character> uniqueMiddle = new HashSet<>();
                // Collect unique middle characters
                for (int i = left + 1; i < right; i++) {
                    uniqueMiddle.add(s.charAt(i));
                }
                // Add count of unique middle characters
